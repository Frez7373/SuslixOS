...
local files={
 ...
 {'apps/desktop.lua','/apps/desktop.lua'},{'apps/bookmarks.lua','/apps/bookmarks.lua'},
...
say('SUSLIXOS INSTALLER',colors.lime); say('Modern desktop OS for CC:Tweaked',colors.lightGray); say('Version '..VERSION,colors.cyan); print()
...
local f=fs.open('/suslix/version','w'); f.write(VERSION); f.close()
local m=fs.open('/suslix/install.info','w'); m.write('SuslixOS '..VERSION..'\nInstalled: '..os.date()); m.close()
say('\nSuslixOS '..VERSION..' installed successfully.',colors.lime); say('Desktop shortcuts and settings are ready.',colors.lightGray); sleep(1); os.reboot()
