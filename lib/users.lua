...
local function hash(s) local h=7; s=tostring(s or ''); for i=1,#s do h=(h*31+s:byte(i))%2147483647 end; return tostring(h) end
...
function M.setPassword(id,password) local d=readFile(path(id)); if not d then return false end; local name=d:match('name=(.-)\n') or id; return writeFile(path(id),'name='..name..'\nhash='..hash(password or '')..'\n') end
